                ...
                <div class='row'>
                    ...
                    <div class='span8'>
                        <table class="table striped hovered dataTable" id="dataTables-1">
                            <thead>
                                <tr>
                                    <th class="text-left">Business Category</th>
                                    <th class="text-left">Total Bought Number</th>
                                </tr>
                            </thead>
                            <tbody>
                            <?php
                            require_once("Includes/db.php");
                            $result = db::getInstance()->get_product_business_by_name($_GET['name']);
                            if (mysqli_num_rows($result) < 1) {
                                exit("The product " . htmlentities($_GET["name"]) . " is not found associate with Business. Please check the spelling and try again");
                            }
                            while ($row = mysqli_fetch_array($result)){                            
                                echo "<tr><td>" . htmlentities($row["Business_Category"]) . "</td>";
                                echo "<td>" . htmlentities($row["Total_Bought"]) . "</td></tr>";
                            }
                            ?>
                          </tbody>
                        </table>
